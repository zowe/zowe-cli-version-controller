    ...
    void version(Map arguments = [:]) {
        ...
        arguments.operation = { String stageName ->
            ...
            if (baseVersion == steps.env.DEPLOY_VERSION) {
                ...
                // Don't let NPM perform Git commit and tag operations, for 2 reasons:
                //  1. We want to commit changes to package-lock.json.
                //  2. We don't like the commit message generated by NPM, so we want to amend the commit. If the commit
                //     has already been tagged, amending it will cause the tag to point to an invalid commit hash.
