...
class NodeJSPipeline extends GenericPipeline {
    ...
    void version(Map arguments = [:]) {
        ...
        arguments.operation = { String stageName ->
            ...
            if (rawPkgVersion == steps.env.DEPLOY_VERSION) {
                gitTag("v$rawPkgVersion", "Create release $rawPkgVersion for ${branch.tag}")
            } else {
                if (!isLernaMonorepo) {
    ...
    void setup(NodeJSSetupArguments arguments) {
        ...
        createStage(name: 'Install Node Package Dependencies', stage: {
            if (steps.sh(returnStatus: true, script: "test -f ${arguments.nvmDir}/nvm.sh") == 0) {
                ...
                steps.sh "n install ${arguments.nodeJsVersion == "--lts" ? "lts" : arguments.nodeJsVersion}"
            } else {
            ...
            try {
                ...
                if (protectedBranches.isProtected(branch)) {
                    ...
                    if (!changeInfo.isPullRequest) {
                        ...
                        ["package.json", "package-lock.json", "npm-shrinkwrap.json"].each {
                            steps.sh "git add ${it} || true"
                        }
                        if (isLernaMonorepo) {
                            runForEachMonorepoPackage(LernaFilter.ALL) {
                                steps.sh "git add package.json || true"
                            }
                        }
